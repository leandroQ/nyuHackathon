...
(function () {
    ...
    WinJS.Namespace.define("FoursquareTrending", publicMembers);
    WinJS.UI.Pages.define("/pages/trending/groupedItems.html", {
        ...
        ready: function (element, options) {
            ...
            $(this.el).find('.search button').click(function (evt) {
                evt.preventDefault();
                self.processVenueTrending();
            })
            this.getCurrentLocation();
        },

        getTrendingVenues: function (ll) {
            var self = this;

            Foursquare.getTrendingVenues({ 'll': '40.7,-74' })
                .done(function (data) {
        ...
        processVenueTrending: function () {
            var value = $('input').val();
            this.getTrendingVenues(value);
        },
        ...
        processVenues: function (data) {
            ...
            FoursquareTrending.itemList = myList;
            document.getElementById('venueListView').winControl.itemDataSource = myList.dataSource;
            myList.notifyReload();
        },
